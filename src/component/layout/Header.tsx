  ...
  return (
    <header className="w-full border-b bg-white shadow-sm">
      <div className="container mx-auto px-4 py-3">
        ...
        <div className="hidden xl:flex items-center justify-between w-full max-w-screen-xl mx-auto">
          {/* Left Logo */}
          <Link
            to="/"
        ...
        <div className="flex flex-col xl:hidden gap-3">
          ...
          <nav className="flex justify-center gap-3 sm:gap-4 flex-wrap sm:flex-nowrap overflow-x-auto px-1 no-scrollbar">
            {[
              { label: "সামাজিক", extra: true },
              { label: "সকল পণ্য", icon: <FaStore className="h-6 w-6" /> },
              {
                label: "অর্ডার কার্ট",
                icon: <FaShoppingCart className="h-6 w-6" />,
              },
              { label: "নতুন অফার", icon: <FaBullhorn className="h-6 w-6" /> },
              { label: "নোটিফিকেশন", icon: <FaBell className="h-6 w-6" /> },
              { label: "প্রোফাইল", icon: <FaUser className="h-6 w-6" /> },
            ].map((item, idx) => (
              <Link
                key={idx}
                to="/"
